...
void main()	{
	...
	uvd.x += .06 * cos(10. * uvd.y + 0.4 * u_time + u_mouse.x / u_resolution.x);
	uvd.y += .05 * sin(10. * uvd.x + 0.4 * u_time + u_mouse.y / u_resolution.y);


	float r = 20.0 * sqrt(dot(uvd, uvd)) + 30.0 * sin(-0.06 * u_time);
	float value = (1.0 * sin(10.0 * r));
	float col = smoothstep(0.5 - 0.2, 0.5, value);



	float line = cos(-.5 * u_resolution.x * uvd.x + 9.1 * u_time);
	line = smoothstep(0.4 - 0.2, 0.5, line);
	
	float rc = 20.0 * sqrt(dot(uvd, uvd));
	float dcircle = smoothstep(0.5 - 0.01, 0.5, 2. / rc);


	float circle = smoothstep(0.5 - 0.01, 0.5, .1 / sqrt(dot(uva, uva)));	

	float summ = 1.0 * (dcircle * line + col * (1.0 - dcircle)) + 0.0 * circle * sin(u_time * 15.0);
